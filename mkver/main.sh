...
if [ ! -f "$NAME.sh" ];then
	if [ -f "main.sh" ];then
		GET_VER_NUM=$(grep -m 1 'VERSION=' main.sh | sed 's/^.*=//')
	else
		echo "Error, missing file $NAME.sh or main.sh!"
		exit 1
	fi
else
...
if [ "$1" == "--git" ];then
	...
	for script in $(ls $PWD/builds/git/ | grep .sh);do
		...
		sed -i '2 i\# Comment:     '"$COMMENT"'' $PWD/builds/git/$script
		if [ "$script" == "$NAME.sh" ] || [ "$script" == "main.sh" ];then
